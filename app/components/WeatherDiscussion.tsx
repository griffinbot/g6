...
export function WeatherDiscussion({ location }: WeatherDiscussionProps) {
  ...
  if (loading && !data) {
    return (
      <div className="p-6">
        <div className="bg-[#0f2237] rounded-2xl p-10 shadow-sm border border-white/10 flex items-center justify-center gap-3 text-slate-400">
  ...
  if ((error && !data) || !data) {
    return (
      <div className="p-6">
        <div className="bg-[#0f2237] rounded-2xl p-8 shadow-sm border border-red-700/40 text-center space-y-3">
  ...
  return (
    <div className="p-6 space-y-6">
      {loading && data && (
        <div className="bg-sky-900/30 border border-sky-700/40 rounded-xl px-4 py-2 text-sm text-sky-300 flex items-center gap-2">
          <Loader2 className="w-4 h-4 animate-spin" />
          Refreshing discussion…
        </div>
      )}
      {error && data && (
        <div className="bg-amber-900/30 border border-amber-700/40 rounded-xl px-4 py-2 text-sm text-amber-300">
          Showing previous discussion while refresh retries.
        </div>
      )}

      <div className="bg-[#0f2237] rounded-2xl p-6 shadow-sm border border-white/10">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h2 className="text-2xl font-semibold mb-2 text-white">{data.title}</h2>
            <p className="text-slate-400">{data.office}</p>
          </div>
        ...
        <div className="flex items-center gap-6 text-sm text-slate-400">
          <div>
            <span className="text-slate-500">Issued:</span> {formatTime(data.issueTime)}
          </div>
          <div>
            <span className="text-slate-500">Office:</span> {data.officeCode}
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0f2237] rounded-2xl p-6 shadow-sm border border-white/10">
        <h3 className="font-semibold mb-4 text-white">Related Resources</h3>
