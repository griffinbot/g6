...
const LS_PREFIX = "wg.forecast.v1.";

function saveCacheToStorage(key: string, data: any): void {
  try {
...
function loadCacheFromStorage(key: string): { data: any; timestamp: number } | null {
  try {
    const raw = localStorage.getItem(LS_PREFIX + key);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed.timestamp !== "number" || !parsed.data) return null;
    // Re-hydrate Date objects serialised to strings by JSON
    if (Array.isArray(parsed.data.hourly)) {
      parsed.data.hourly = parsed.data.hourly.map((h: any) => ({ ...h, time: new Date(h.time) }));
    }
    if (Array.isArray(parsed.data.daily)) {
      parsed.data.daily = parsed.data.daily.map((d: any) => ({ ...d, date: new Date(d.date) }));
    }
    return parsed;
  } catch {
    return null;
...
export function useWeather(lat: number, lon: number) {
  ...
  const fetchWeather = useCallback(async () => {
    ...
    // ------------------------------------------------------------------
    // Step 1: Serve cached data immediately for instant rendering.
    //   - In-memory cache is fastest (no parse overhead).
    //   - localStorage cache survives page refresh.
    //   Both paths set loading:false so the UI shows data right away.
    //   We then ALWAYS continue to a background network fetch so data
    //   stays validated (stale-while-revalidate on every call).
    // ------------------------------------------------------------------
    let servedFromCache = false;

    const memCached = cache[key];
    if (memCached) {
      setState({
        ...memCached.data,
        loading: false,
        error: null,
        lastUpdated: new Date(memCached.timestamp),
      });
      servedFromCache = true;

      // If in-memory data is still fresh, skip the network call entirely.
      if (now - memCached.timestamp < CACHE_DURATION) return;
    }

    if (!servedFromCache) {
      // Check localStorage (slower but persistent across refreshes)
      const stored = loadCacheFromStorage(key);
      if (stored) {
        // Populate in-memory cache so next call is instant
        cache[key] = stored;
        // Always show cached data immediately (loading:false)
        setState({
          ...stored.data,
          loading: false,
          error: null,
          lastUpdated: new Date(stored.timestamp),
        });
        servedFromCache = true;

        // If localStorage data is also still fresh, skip network.
        if (now - stored.timestamp < CACHE_DURATION) return;
      }
    }
    ...
    // ------------------------------------------------------------------
    // Step 2: Background (or foreground) network fetch.
    //   If we already served cached data we don't show a loading spinner,
    //   we just update silently when fresh data arrives.
    // ------------------------------------------------------------------
    ...
    try {
      ...
      const elevation_ft = Math.round((data.elevation ?? 0) * 3.28084);
      const result = { current, hourly, daily, elevation_ft };
      const now = Date.now();
      cache[key] = { data: result, timestamp: now };
      saveCacheToStorage(key, result);
      ...
      setState((prev) => ({
        ...prev,
        loading: false,
        // Only surface error if we have no data to show
        error: prev.current ? null : (err.message || "Failed to fetch weather data"),
      }));
    }
  }, [lat, lon]);
  ...
  useEffect(() => {
    fetchWeather();
    const interval = setInterval(fetchWeather, AUTO_REFRESH_INTERVAL);
    return () => {
