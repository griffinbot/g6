...
export const NORMALIZED_ALTITUDES_AGL = [
  ...
  8000, 9000, 10000, 12000, 14000, 16000, 18000, 20000, 25000, 30000, 34000,
];
...
// ---------------------------------------------------------------------------
// React hook — module-level cache shared across all instances
// ---------------------------------------------------------------------------
const windAloftCache: Record<string, { data: { hours: WindAloftHour[]; elevation_m: number }; timestamp: number }> = {};
const WIND_ALOFT_CACHE_DURATION = 5 * 60 * 1000; // 5 minutes
...
export function useWindAloft(lat: number, lon: number): WindAloftState & { refetch: () => void } {
  ...
  const fetchData = useCallback(async () => {
    ...
    const cached = windAloftCache[cacheKey];
    if (cached && Date.now() - cached.timestamp < WIND_ALOFT_CACHE_DURATION) {
    ...
    try {
      ...
      windAloftCache[cacheKey] = { data: parsed, timestamp: Date.now() };
